...
messageParentsForm.addEventListener('submit', (e) => {
    ...
    if(document.querySelector('#schedDate').value > today){
        ...
        showSnackbar("selected date is invalid");
    }
})
...
function checkMinorViolationCount(){
    ...
    .then((json) => {
        ...
        if((json["violationID"].length % 3) == 0){
            for(let i = 0; i < rowMinorReset; i++){
                document.querySelector("#studentViolations").innerHTML = '';
            }
            for(let i = 0; i < 3; i++){
...
function getViolationCase(){
    ...
    .then((json) => {   
        ...
        for(let i = 0; i < json["violationCase"].length ; i++){
            ...
            opt.text = json["violationCase"][i]
            opt.value = json["violationCase"][i]
            document.querySelector('#searchCase').options.add(opt);
        }
    })
}
...
sanctionBatch.addEventListener('click', (e) => {
    e.preventDefault()
    const input = document.getElementById("sanctionExcel");
    readXlsxFile(input.files[0]).then(function (data) {
        const headers = data[0];
        const jsonData = [];
        for (let i = 1; i < data.length; i++) {
        ...
        for(i = 0; i < jsonData.length; i++){
            const formData = new FormData();
            formData.append("violationID", jsonData[i]["Violation ID"]);
            formData.append("sanction", jsonData[i]["Sanction"]);
